...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.n_encode_layers: int = n_encode_layers
        self.temp: float = 1.0
    ...
    def forward(self, input: torch.Tensor, deterministic: bool = False, evaluate_action: bool = False,
                normFactor: int = 1, evaluate: bool = False)-> Tuple[
        ...
        graph_size: int = input.size(1)
        internal_nodes_size: int = internal_nodes.size(1)
        leaf_node_size: int = leaf_nodes.size(1)
        next_size: int = next_item.size(1)
    ...
    def _inner(self, embeddings: torch.Tensor, mask: torch.Tensor, shape: Tuple[int,int,int], full_mask: torch.Tensor,
               ...
               deterministic: bool = False, evaluate_action: bool = False) -> Tuple[
    ...
    def _precompute(self, embeddings: torch.Tensor, full_mask: torch.Tensor, shape: Tuple[int,int,int], valid_length: torch.Tensor,
