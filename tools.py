...
def observation_to_node(observation: torch.Tensor, internal_node_holder: int, internal_node_length: int,
                        leaf_node_holder: int) -> typing.Tuple[torch.Tensor,
    torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    internal_nodes = observation[:, 0:internal_node_holder, 0:internal_node_length]
    leaf_nodes = observation[:, internal_node_holder:internal_node_holder + leaf_node_holder, 0:8]
    current_box = observation[:, internal_node_holder + leaf_node_holder:, 0:6]
    valid_flag = observation[:, internal_node_holder: internal_node_holder + leaf_node_holder, 8]
    full_mask = observation[:, 0:internal_node_holder + leaf_node_holder, -1]
    position = observation[:, 0:internal_node_holder + leaf_node_holder, 0:3]
    return position, internal_nodes, leaf_nodes, current_box, valid_flag, full_mask
...
def get_args():
    ...
    parser.add_argument('--lnes', type=str, default='BP',
                        help='Leaf Node Expansion Schemes:BP(recommend), EMS , EV, EP, CP, FC')
    ...
    parser.add_argument('--learning-rate', type=float, default=1e-6, metavar='η',
    ...
    parser.add_argument('--embedding-size', type=int, default=126, help='Dimension of input embedding')
